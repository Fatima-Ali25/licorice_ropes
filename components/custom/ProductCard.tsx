...
const ProductCard = ({ product, onAddToCart, className }: ProductCardProps) => {
  ...
  return (
    <Link href={`/product/${product.id}`} className="w-full max-w-xs sm:w-64 md:w-72 flex items-center justify-center">
      <Card className={`w-full max-w-xs sm:w-64 md:w-72 md:items-center lg:w-80  h-auto sm:h-72 md:h-96 border-0 shadow-sm hover:shadow-md transition-all duration-300 group cursor-pointer hover:scale-105 ${className}`}>
